...
### 方式一：Plugin Marketplace（推荐，两条命令）
...
脚本将 `SKILL.md` 复制到 `~/.claude/skills/save-project-memory/`，并可选写入 `MEMPALACE_BASE_DIR` 环境变量。**mempalace 本身由 Skill 在首次使用时自动安装（约 30–60 秒，仅需一次）。**
...
```
<your-project-name>/       ← wing = 项目目录名，多项目互不干扰
